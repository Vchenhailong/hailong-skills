...
| 字幕区 | [-6.8, 6.8] | [-3.85, -2.8] | 字幕专用区（可容纳2行字幕，高度约1.05单位，font_size=18） |
| 主内容区 | [-6.75, 1.35] | [-2.5, 3.0] | 公式、推导步骤、文字说明（两栏时） |
| 图形区 | [1.85, 6.75] | [-2.5, 3.0] | 几何图形、坐标系、图表（两栏时） |
```
...
| ------------------------ | -------------- | ---------- | ----------- | ----------------- | ----------- |
| 三区（标题+主内容+字幕） | 2:7:1          | 1/10 = 10% | ≈ 0.68 单位 | Y ∈ [-3.48, -2.8] | ≈ 5.95 单位 |
| 两区（主内容+字幕）      | 9:1            | 1/10 = 10% | ≈ 0.68 单位 | Y ∈ [-3.48, -2.8] | ≈ 6.12 单位 |

> **三区说明**：当标题区存在时，安全区 Y ∈ [-2.8, 4.0]（高 6.8 单位）按 2:7:1 分配 → 标题约 1.36 单位（Y ∈ [2.64, 4.0]），内容约 4.76 单位（Y ∈ [-2.12, 2.64]），字幕约 0.68 单位（Y ∈ [-3.48, -2.8]）。
...
2. 方式二：同一原子内强制垂直排列
   ...
   {"text": "A = \\begin{bmatrix} a*{11} & a*{12} \\\\ a*{21} & a*{22} \\end{bmatrix}", "type": "formula"},
