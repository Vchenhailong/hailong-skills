...
3. .shift() - 仅允许用于整体位置调整（如将整个 VGroup 移动到起始高度），禁止对单个元素使用
   group.shift(UP _ 2.0) # 允许
   text.shift(LEFT _ 0.5) # 禁止
...
- 底部 Y ∈ [-3.85, -2.8] 为字幕专用区
- 所有教学内容的最低 Y ≥ -2.8
- 布局安全区域基于 16:9 比例，通过 `ZoneConstants.compute()` 动态计算

> **旧版固定值**：早期版本使用固定坐标（屏幕 15×9，安全区 Y ∈ [-2.8, 4.0]）。新版本使用 ZoneConstants.compute() 动态计算，推荐迁移。

## 3. 分区布局与顶部居中规范（强制）

### 3.1 安全边界定义

标准配置 `config.frame_height=8`（16:9），中心原点

```
| 边界 | X 范围 | Y 范围 | 说明 |
|------|--------|--------|------|
| 屏幕边界 | [-7.11, 7.11] | [-4.0, 4.0] | 全屏范围 |
| 安全区域 | [-6.75, 6.75] | [-2.8, 4.0] | 内容安全区（字幕区上界 Y=-2.8） |
| 字幕区 | [-6.8, 6.8] | [-3.85, -2.8] | 字幕专用区（高度由 ZoneConstants.compute() 动态确定） |
| 主内容区 | [-6.75, 1.35] | [-2.5, 3.11] | 公式、推导步骤、文字说明（两栏时） |
| 图形区 | [1.85, 6.75] | [-2.5, 3.11] | 几何图形、坐标系、图表（两栏时） |
```
...
| 字幕区 | 居中（系统自动） | 底部 Y = -3.85（2行 + 行间距） | 语音字幕（固定，防止抖动） |
```
...
|   （步骤说明）    |      （公式）       |      （图形）           |
|                   |                    |                         |
| X ∈ [-6.75, -2.2] | X ∈ [-1.7, 2.35]   | X ∈ [2.85, 6.75]        |
...
| ------------------------ | -------------- | ---------- | ----------- | ----------------- | ----------- |
| 三区（标题+主内容+字幕） | 2:7:1          | 1/10 = 10% | ≈ 0.72 单位 | Y ∈ [-3.6, -2.88] | ≈ 4.94 单位 |
| 两区（主内容+字幕）      | —:9:1          | 1/10 = 10% | ≈ 0.72 单位 | Y ∈ [-3.6, -2.88] | ≈ 6.38 单位 |
...
|------|------|----------|-------------------|------|
| 单栏 | 1 | 纯推导、纯图形 | 全宽 [-6.75, 6.75] | — |
| 两栏 | 2 | 图形（右）+ 推导/文字（左） | 左 [-6.75, 1.35]，右 [1.85, 6.75] | 60% / 40% |
| 三栏 | 3 | 步骤说明（左）+ 公式（中）+ 图形（右） | 左 [-6.75, -2.2]，中 [-1.7, 2.35]，右 [2.85, 6.75] | 30% / 30% / 40% |
| 两栏对比 | 2 (并列) | 前后对比、错误 vs 正确 | 左 [-6.75, -0.05]，右 [0.55, 6.75] | 50% / 50% |
```
...
| 栏位 | 内容 | 宽度 | X 范围 |
|------|------|------|--------|
| 左栏 | 操作步骤编号（1. 取A第i行；2. 取B第j列；3. 点积） | 3.55 单位 | [-6.75, -2.2] |
| 中栏 | 公式 (AB)_{ij} = Σ_k A_{ik}B_{kj} | 4.05 单位 | [-1.7, 2.35] |
| 右栏 | 矩阵 A、B 的图形（高亮对应行列） | 3.9 单位 | [2.85, 6.75] |
```
...
| 参数             | 值               | 说明                             |
| ---------------- | ---------------- | -------------------------------- |
| 屏幕宽高         | 14.22 × 8.0 单位 | 标准 16:9 配置（frame_height=8） |
| 安全区域 X       | ±6.75            | 留出 0.71 边距（5%）             |
...
- 左栏 X 范围 [-6.75, 1.35] -> 宽度 8.1 单位（60%）
- 右栏 X 范围 [1.85, 6.75] -> 宽度 4.9 单位（40%）
- 栏间距 0.5 单位
- 左栏最大字符数 = 8.1 ÷ 0.6 = 13.5 字符（按 0.6 单位/字符计）
...
two_left_w = zones["two_left_x_max"] - zones["two_left_x_min"]
two_right_w = zones["two_right_x_max"] - zones["two_right_x_min"]
```
...
| 右栏 | 1（两栏）/ 2（三栏） | RIGHT（右对齐） | 内容右边缘对齐栏右边界 `x_max` |
...
# 两栏模式：左栏左对齐，右栏右对齐
columns = ZoneConstants.compute_column_layout(zones, num_columns=2, has_graphics=True)

# 左栏内容（index=0 → LEFT）
ZoneConstants.align_content_in_column(left_content, columns[0], 0)

# 右栏内容（index=1 → 两栏右栏 → RIGHT）
ZoneConstants.align_content_in_column(right_content, columns[1], 1)
```

#### 10.5.4 Manim 实现要点

- 使用 `RIGHT` / `LEFT` 常量作为对齐参考边
...
bad = Circle(color=BLUE).move_to([3, -2, 0])
bad = Text("标题").shift(RIGHT * 4.5 + UP * 2.3)

# ✅ DO：使用 axes.c2p() 转换坐标系，使用 VGroup.arrange() 相对布局
good = Circle(color=BLUE).move_to(axes.c2p(3, -2))
good_title = VGroup(title_text, subtitle).arrange(DOWN, center=True)
```
...
bad = obj1.move_to(ORIGIN)
bad = obj2.shift(RIGHT * 2)
bad = obj3.shift(RIGHT * 2 + UP * 0.5)    # 错位累积无法批量修正

# ✅ DO：整体 VGroup + .arrange() / .next_to()
good = VGroup(obj1, obj2, obj3).arrange(RIGHT, buff=1.0)
```
...
```python
# ❌ DON'T：直接放置元素到画布边缘
bad = Circle().move_to(RIGHT * 6.75)   # 超出了安全边界（安全区右边界 X = 6.75）

# ✅ DO：所有放置逻辑必须通过 place_in_safe_area()
good = Circle().move_to(safe_place(right=0.8, vertical="top"))
```
...
bad_scene = Scene:     # 无布局边界检查，无 validate_layout()
...
```python
# ❌ DON'T：手动估算中点
bad_x = (point_a[0] + point_b[0]) / 2  # 硬编码算术平均
bad_x = 1.5                              # 直接写数字

# ✅ DO：使用 Midpoint 类推导
mid = Midpoint(start_point, end_point)
mid_coord = axes.c2p(mid[0], mid[1])
```
...
good_formula = MathTex(r"...").move_to(axes.c2p(0, 2))
```
...
violations = self.validate_layout(mobjects)  # 报告物体-液体重叠

# ✅ DO：浮力场景配置 overlap_patterns
violations = self.validate_layout(
    mobjects,
    allowed_overlap_patterns=ALLOWED_PATTERNS  # 包含 object_submerged_in_liquid
)
```
...
```python
# ❌ DON'T：引用已废弃的路径
from scripts.layout_base import LayoutScene       # ❌ 路径已变更
from scripts.safe_place import safe_place         # ❌ 方法已重命名

# ✅ DO：使用当前有效的脚本路径和方法名
from scripts.layout.scene_base import LayoutScene
scene.place_in_safe_area(obj)                    # ✅ 新方法名
```
