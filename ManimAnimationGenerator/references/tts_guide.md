...
在传给 TTS 的文本中，必须将以下符号替换为中文读音。转义实现调用 `scripts/tex_tools.py` 中的 `math_symbols_to_speech()` 函数。
...
| \sqrt           | 根号     | \sqrt{a}            | 根号 a           |
| \sum            | 求和     | \sum\_{i=1}^n       | 求和 i 从 1 到 n |
| \int            | 积分     | \int f(x)dx         | 积分 f x d x     |
| \prod           | 连乘     | \prod\_{i=1}^n      | 连乘 i 从 1 到 n |
| \lim            | 极限     | \lim\_{x \to 0}     | 极限 x 趋向于 0  |
...
云端 TTS 集成方式及 SSML 语法规范详见 manim-voiceover 官方文档。
...
每月 50 万字符（约 500 分钟语音）。注册地址：https://azure.microsoft.com/zh-cn/services/cognitive-services/text-to-speech/
...
云端 TTS 包安装命令见 manim-voiceover 官方安装页面。
...
安装完成后，通过 manim-voiceover 官方示例脚本验证安装是否成功。
...
- **禁止**：直接发送 LaTeX 给 TTS 引擎（如 `lim_{x→0} sin(x)/x = 1`）
- **必须**：调用 `scripts/tex_tools.py` 中的 `math_symbols_to_speech()` 映射后再发送给 TTS
...
- **禁止**：TTS 文本中出现 `$...$` 等 LaTeX 分隔符
- **必须**：TTS 前清除所有 LaTeX 分隔符
...
- **禁止**：highlight_range 指定了不存在的字符位置
- **必须**：highlight_range 必须覆盖 content 中 formula 项的实际字符范围
...
- **禁止**：所有 step 的 duration 使用固定值（如全部 6 秒）
- **必须**：每步根据 tts_text 实际字数估算朗读时长（约 4 字/秒）
