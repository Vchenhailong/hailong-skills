...
class GraphicsZone(ZoneBase):
    ...
    def place_content(self, graphics_group: VGroup) -> VGroup:
        ...
        # 防止图形底部进入字幕区（layout.md 第 7.1 节第 5 条）
        bottom_edge = graphics_group.get_bottom()[1]
        subtitle_safe_y = ZoneConstants.SUBTITLE_ZONE_Y_MAX + 0.3
        if bottom_edge < subtitle_safe_y:
            from manim import UP
            shift_amount = subtitle_safe_y - bottom_edge
            graphics_group.shift(UP * shift_amount)
