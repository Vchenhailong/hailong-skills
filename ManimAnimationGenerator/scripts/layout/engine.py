...
class LayoutEngine:
    ...
    def decide(
        ...
        if estimated_height > ZoneConstants.VERTICAL_OVERFLOW_THRESHOLD:
            if estimated_width < ZoneConstants.HORIZONTAL_OVERFLOW_THRESHOLD:
                return LayoutDecision(mode=LayoutMode.TWO_COLUMN, should_scale=True,
                                      scale_factor=ZoneConstants.VERTICAL_OVERFLOW_THRESHOLD / estimated_height)
            else:
    ...
    def arrange_content(
        ...
        if mode == LayoutMode.VERTICAL:
            ...
            if right_objs:
                right_col = VGroup(*right_objs).arrange(DOWN, buff=buff, aligned_edge=LEFT)
                group = VGroup(left_col, right_col).arrange(RIGHT, buff=ZoneConstants.ELEMENT_BUFF)
            else:
                group = left_col
            ...
            left_col = VGroup(*left_objs).arrange(DOWN, buff=buff * 0.8, aligned_edge=LEFT)
            mid_col = VGroup(*mid_objs).arrange(DOWN, buff=buff, aligned_edge=LEFT) if mid_objs else VGroup()
            right_col = VGroup(*right_objs).arrange(DOWN, buff=buff * 0.8, aligned_edge=LEFT) if right_objs else VGroup()
