...
class LayoutOptimizer:
    ...
    # 字体大小限制
    MIN_FONT_SIZE = ZoneConstants.MIN_CONTENT_FONT_SIZE  # = 24
    MAX_FONT_SIZE = 34
    SCALE_FACTOR_PER_ROUND = 0.9  # 每轮缩放系数

    # 图形缩放下限（防止内容过小导致无法渲染）
    MIN_GRAPHICS_SCALE = ZoneConstants.MIN_GRAPHICS_SCALE_RATIO  # = 0.80

    # 任意图元线宽最小值（points）
    MIN_STROKE_WIDTH = ZoneConstants.MIN_STROKE_WIDTH_POINTS  # = 3.0
    ...
    def _apply_scale(
        ...
        对图形类对象缩放时，scale_factor 不低于 MIN_GRAPHICS_SCALE（=0.80）。
        对所有 Mobject 强制应用 stroke_width 下限（=3 points），防止缩放后线宽过细。
        ...
        for mobj in mobjects:
            if isinstance(mobj, (Text, MathTex)):
                ...
                # 图形类：只对 OVERFLOW 违规进行缩放
                # CENTER_OFFSET 等位置违规不应缩放图形，只应调整位置
                violation_type = violation.get("type", "")
                if violation_type in ("WIDTH_OVERFLOW", "HEIGHT_OVERFLOW"):
                    current_scale = getattr(mobj, "_scale_factor", 1.0) or 1.0
                    target_scale = current_scale * self.SCALE_FACTOR_PER_ROUND
                    if target_scale < self.MIN_GRAPHICS_SCALE:
                        target_scale = self.MIN_GRAPHICS_SCALE
                    if target_scale < current_scale:
                        mobj.scale(target_scale, about_point=mobj.get_center())
                        scaled_count += 1
                else:
                    logging.debug(f"[_apply_scale] 跳过图形 {type(mobj).__name__}，违规类型={violation_type} 不支持图形缩放")

            # 对所有 Mobject（含文本和图形）强制应用 stroke_width 下限
            self._enforce_min_stroke_width(mobj)

        return scaled_count > 0

    def _enforce_min_stroke_width(self, mobj: Mobject) -> None:
        """强制设置 Mobject 的 stroke_width 不低于 MIN_STROKE_WIDTH（=3 points）

        即使经过缩放，线宽也不能进一步减小此最小值。
        递归应用到所有子对象。
        """
        POINTS_PER_UNIT = 72.0
        min_stroke_units = self.MIN_STROKE_WIDTH / POINTS_PER_UNIT
        try:
        ...
        for i, mobj in enumerate(mobjects):
            if isinstance(mobj, Text):
                if use_minipage:
                    ...
                    wrapped = self._wrap_text_with_minipage(mobj, target_width)
                    if not wrapped:
                        # 失败回退到 Pango 路径
    ...
    def _wrap_text_object(self, text_obj: Text, target_width: float) -> bool:
        ...
        try:
            ...
            new_text = Text(
                ...
                line_spacing=0.8,  # 行间距 ×0.8，改善换行后的阅读体验
            )
    ...
    def _compute_em_width(self, manim_width: float, font_size: int) -> float:
    ...
    def _escape_text_for_latex(self, text: str) -> str:
        """转义 LaTeX 特殊字符
        ...
        result = []
        for ch in text:
            if ch in self._LATEX_SPECIAL_ESCAPES:
                result.append(self._LATEX_SPECIAL_ESCAPES[ch])
            else:
                result.append(ch)
        return "".join(result)
    ...
    def _wrap_text_with_minipage(
        ...
        # 步骤 1: 算 em 宽度
        em_w = self._compute_em_width(target_manim_width * 0.95, font_size)

        # 步骤 2: 转义 + 构建 LaTeX
        # \fontsize{size}{baselineskip}\selectfont 设置行距（1.4×字号的 baselineskip）
        # \raggedright 左对齐（禁止两端对齐导致的过度词间距）
        safe_text = self._escape_text_for_latex(original_text)
        latex_src = (
            rf"\begin{{minipage}}[t]{{{em_w:.3f}em}}"
            rf"\fontsize{{{font_size:.0f}}}{{{font_size*0.8:.0f}}}\selectfont"
            rf"\raggedright "
            rf"{safe_text}"
            rf"\end{{minipage}}"
        )
