...
def is_pure_latex_formula(text: str) -> bool:
    ...
    latex_patterns = [
        ...
        r'\\overline', r'\\text\{', r'\\vec', r'\\neq',
        r'\\Rightarrow', r'\\Leftrightarrow', r'\\sum', r'\\int',
        r'\\alpha', r'\\beta', r'\\gamma', r'\\theta',
        r'\\matrix', r'\\begin\{pmatrix\}', r'\\begin\{bmatrix\}',
        r'_\{', r'\^\{', r'\\\[', r'\\\]'
    ]
...
def split_mixed_content(text: str) -> List[Dict[str, str]]:
    ...
    while current_pos < length:
        ...
        if begin_match:
            ...
            if end_match:
                formula_end = formula_start + begin_match.end() + end_match.start() + end_match.end()
                result.append({
                    "text": text[formula_start:formula_end],
                    "type": "formula"
                })
...
def validate_and_fix_atom(atom: Dict[str, Any], fix: bool = True) -> Tuple[bool, List[str], Dict[str, Any]]:
    """
    校验并修复单个原子对象
    ...
    if "layout" in atom:
        if atom["layout"] not in VALID_LAYOUT_TYPES:
            errors.append(f"layout 值 '{atom['layout']}' 不合法，合法值: {VALID_LAYOUT_TYPES}")
...
def validate_and_fix_json(data: Dict[str, Any], fix: bool = True) -> Tuple[bool, List[str], Dict[str, Any]]:
    """
    校验并修复整个 JSON 数据
...
def process_file(input_path: str, output_path: Optional[str] = None, 
                 fix: bool = True, verbose: bool = True) -> bool:
    """
    处理单个 JSON 文件
...
def main():
    ...
    parser.add_argument("--output", "-o", type=str, help="输出 JSON 文件路径（默认覆盖输入）")
    parser.add_argument("--batch", "-b", action="store_true", help="批量处理模式")
    parser.add_argument("--dir", "-d", type=str, default="courses/", help="批量处理时指定的目录")
    parser.add_argument("--validate-only", "-v", action="store_true", help="仅校验，不修复")
